...
rps_thread_proxy_run(rps_http_request_t *r, rps_upstream_t *u)
{
    ...
    {
        int sock_err = 0; socklen_t len = sizeof(sock_err);
    ...
    while (!u->header_complete) {
        ...
        n = recv(fd, u->response_buf->last, (size_t)(u->response_buf->end - u->response_buf->last), 0);
        if (n <= 0) { 
            rps_upstream_finalize(r, RPS_ERROR); 
            return RPS_ERROR; 
        }
    ...
    /* 收 body：直接读入 response_buf */
    while (u->body_received < u->content_length_n) {
        if (rps_thread_poll_wait(fd, POLLIN, POLL_TIMEOUT) <= 0){ 
            rps_upstream_finalize(r, RPS_ERROR); 
            return RPS_ERROR; 
        }
        n = recv(fd, u->response_buf->last,
                 (size_t)(u->response_buf->end - u->response_buf->last), 0);
        if (n == 0) break;
        if (n < 0) { 
            rps_upstream_finalize(r, RPS_ERROR); 
            return RPS_ERROR; 
...
rps_thread_ws_start(rps_http_request_t *r, rps_upstream_t *u)
{
    ...
    /* 透传 101 给客户端 */
    {
        ...
        if (rps_thread_blocking_send_all(r->connection->fd, u->response_buf->pos, len, POLL_TIMEOUT) != 0){ 
            rps_upstream_finalize(r, RPS_ERROR); 
            return RPS_ERROR; 
        }
    }
